        ...
        <div className="w-full flex-1 relative min-h-[180px]">
          {isGif ? (
            <img
              ...
              className={`absolute inset-0 w-full h-full ${['chill_vibes', 'lofi_girl', 'anime_room'].includes(activePetType || '') ? 'object-contain' : 'object-cover'}`}
            />
          ) : (
            <video
              key={petMediaSrc}
                ...
                return (
                  ...
                  >
                    <div className="w-full aspect-square rounded-xl overflow-hidden relative bg-black/40">
                      {isItemGif ? (
                        <img src={char.src} alt={char.name} className={`w-full h-full ${['chill_vibes', 'lofi_girl', 'anime_room'].includes(char.id) ? 'object-contain' : 'object-cover'} group-hover:scale-110 transition-transform duration-500`} />
                      ) : (
                        <video src={char.src} muted loop autoPlay playsInline className={`w-full h-full ${['chill_vibes', 'lofi_girl', 'anime_room'].includes(char.id) ? 'object-contain' : 'object-cover'} group-hover:scale-110 transition-transform duration-500`} />
                      )}
